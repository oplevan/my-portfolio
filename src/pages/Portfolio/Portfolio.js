    ...
    <>
      <Box component='main'>
        ...
        <Container maxWidth='lg' component='section'>
          <h1>Portfolio</h1>
          <p align='left'>From small Web Applications and UI/UX animations to ReactJS, Redux Apps. Check out my latest web development portfolio projects.</p>
          <div className='filters'>
            {filters.map((item) => (
              <Button
                ...
                title={item.title}
                filter={item.filter}
                clickHandler={{ func: filterItems, args: item.id }}
                extraClassName={filter === item.id && 'active'}
              />
            ))}
            {filter === 'all' ? (
